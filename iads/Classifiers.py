...
def validation_croisee(C, DS, nb_iter):
    ...
    for i in range(nb_iter):
        X_train, Y_train, X_test, Y_test = ut.crossval_strat(X, Y, nb_iter, i)
        newC = copy.deepcopy(C)
...
def leave_one_out(C, DS):
    ...
    for i in range(n):
        ...
        # Retirer l'exemple testé du dataset
        desc_temp = np.delete(desc, i, axis=0)
        label_temp = np.delete(label, i)
        
        # Entrainer le classifieur sur le dataset sans l'exemple testé
        newC.train(desc_temp, label_temp)
        
        # Tester le modèle sur l'exemple testé
        y_pred = newC.predict(x_test)
        if y_pred == y_test:
            compte += 1
...
class NoeudNumerique:
    ...
    def classifie(self, exemple):
        ...
        print(exemple[self.attribut])
        print(exemple)
        print(type(exemple[self.attribut].tolist()))
        print(type(exemple[self.attribut].tolist()[0]))
        print(str(exemple[self.attribut].tolist()[0]))
        
        if (exemple[self.attribut]) <= self.seuil:
            # descente récursive dans le noeud associé à la valeur de l'attribut
            # pour cet exemple:
            return self.Les_fils[0].classifie(exemple)
        else:
            return self.Les_fils[1].classifie(exemple)
...
def construit_AD_num(X,Y,epsilon,LNoms = []):
    ...
    if (entropie_classe <= epsilon) or  (nb_lig <=1):
        ...
        gain_max = 0.0  # meilleur gain trouvé 
        ...
        for i in range(X.shape[1]):  # Parcours de tous les attributs
            ...
            for v in valeurs:
                Yv = Y[X[:, i]==v]  # Construction de l'ensemble des labels de v
                entropie_cond += len(Yv)/len(Y)*entropie(Yv)  # Calcul de l'entropie conditionnel
            gain_info = entropie_classe - entropie_cond
            if gain_info > gain_max:
                gain_max = gain_info
                i_best = i
                Xbest_valeurs = valeurs
                (Xbest_seuil, _), (_,_) = ut.discretise(X,Y,i)
                if Xbest_seuil == None:
                    Xbest_tuple = ((X,Y), (None, None))
                else:
                    Xbest_tuple = ut.partitionne(X,Y,i,Xbest_seuil)
